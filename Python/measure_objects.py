...
def getWidthAndHeightFromContours(cnt, pixel_per_m : int | float):
...
def addSingleCntAndSizesToImage(numpy_ndarray_image, cnt, pixel_per_m : int | float):
    box = cv2.minAreaRect(cnt)
    box = cv2.boxPoints(box)
    box = np.array(box, dtype="int")
    box = perspective.order_points(box)
...
def measureEveryObject(cnts, pixel_per_m : int | float):
    returnArray = []
...
def addAllCntsAndSizesToImage(cnts, pixel_per_m : int | float, input_img_path : str, output_img_path : str):
...
def test():
    ...
    pixel_per_m = getPixelPerCmByImageWidth(my_img, REAL_WIDTH_CM)
    # pixel_per_m = getPixelPerCmByImageHeight(my_img, REAL_HEIGHT_CM)
    
    # get contours
    # if not found any, object type is None
    cnts = getContoursSortedFromImage(my_img)

    addAllCntsAndSizesToImage(cnts, pixel_per_m, my_img, output_img)


    measured_obj_list = measureEveryObject(cnts, pixel_per_m)
...
def alladyn_main(source_img_path : str, REAL_VISIBLE_WIDTH_AT_IMG_IN_M : int | float, output_img_path : str | None):
    ...
    if cnts:
        ...
        resultData = {
            ...
            'height' : roundMeasurements(largest_obj_real_height),
        }
            
        resultData['area'] = roundMeasurements(resultData["height"] * resultData['width'])
    else:
        resultData = {'width': 0.0 , 'height': 0.0, 'area' : 0.0}
...
def demo():
    ...
    REAL_VISIBLE_WIDTH_AT_IMG_IN_M = 2.0
    
    
    print(str(alladyn_main(my_img, REAL_VISIBLE_WIDTH_AT_IMG_IN_M, output_img_path)))
